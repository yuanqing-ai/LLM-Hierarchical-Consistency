...
def test_internvl(json_file, output_file, prompt_order,internvl_3):
    ...
    for i, entry in enumerate(tqdm(test_data, desc="Processing")):
        ...
        for t, (level_key, choices_key) in enumerate(zip(level_keys, choices_keys)):
            ...
            if prompt_order == 0:
                prompt_template = "What is the taxonomic category of the animal in this image?"
            elif prompt_order == 1:
                prompt_template = "How can the animal in this image be categorized in taxonomy?"
            elif prompt_order == 2:
                prompt_template = "Based on classification, what type of animal is this?"
            elif prompt_order == 3:
                prompt_template = "What is the hierarchical class of the animal shown here?"
            else:
                prompt_template = "Where does this animal belong in the taxonomic hierarchy?"
