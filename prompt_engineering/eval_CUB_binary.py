...
def test_qwen(json_file, output_file, prompt_order,model_path):
    ...
    for i, entry in enumerate(tqdm(test_data, desc="Processing")):
        ...
        for t, (level_key, choices_key) in enumerate(zip(level_keys, choices_keys)):
            ...
            for item in choices:
                if t == 0:
                    prompt_template = f"The bird in the image belongs to the order {item}."
                elif t == 1:
                    prompt_template = f"The bird in the image belongs to the family {item}."
                elif t == 2:
                    prompt_template = f"The bird in the image belongs to the genus {item}."
                else:
                    prompt_template = f"The bird in the image belongs to the species {item}."
            
                binary_ans, prediction = infer_level(image_path, prompt_template, choice_map, processor, model)
                result_entry[f"{item}_predction"] = prediction
                result_entry[f"{item}"] = binary_ans
...
def infer_level(image_path, prompt_template, choice_map, processor, model):
    ...
    {
        "role": "system",
        "content": "You are an expert in hierarchical image classification. Given an image and a corresponding statement, determine whether the statement accurately reflects the content of the image. Respond with a single word: Yes or No."
    },
    {
        "role": "user",
        "content": [
...
if __name__ == "__main__":
    ...
    # json_file = "/projectnb/ivc-ml/yuwentan/LLaVA-NeXT/Evaluate_CLS/data/CUB200/CUB200_with_random_choice.jsonl"

    test_qwen(args.test_set, args.output_file, args.prompt_order,args.model_path)