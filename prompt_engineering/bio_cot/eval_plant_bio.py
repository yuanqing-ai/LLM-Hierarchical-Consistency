    ...
    for i, entry in enumerate(tqdm(test_data, desc="Processing")):
        ...
        for t, (level_key, choices_key) in enumerate(zip(level_keys, choices_keys)):
            ...
            if prompt_order == 0:
                ...
                if t == 0:
                        prompt_template = "Given the plant in the image, what is its taxonomic classification at the kingdom level?"
                elif t == 1:
                        prompt_template = "Given the plant in the image, what is its taxonomic classification at the phylum level?"
                elif t == 2:
                        prompt_template = "Given the plant in the image, what is its taxonomic classification at the class level?"
                elif t == 3:
                        prompt_template = "Given the plant in the image, what is its taxonomic classification at the order level?"
                elif t == 4:
                        prompt_template = "Given the plant in the image, what is its taxonomic classification at the family level?"
                elif t == 5:
                        prompt_template = "Given the plant in the image, what is its taxonomic classification at the genus level?"
                else:
                        prompt_template = "Given the plant in the image, what is its taxonomic classification at the species level?"
            elif prompt_order == 2:
                    prompt_template = "What could the plant in the image be classified as?"
            elif prompt_order == 3:
                    prompt_template = "How can the plant in the image be taxonomically categorized?"
            else:
                    prompt_template = "What is the systematic position of the plant in the image in the biological classification hierarchy?"
...
def infer_level(image_path, prompt_template, choice_map, processor, model):
    ...
    system_prompt = "You are an expert in hierarchical image classification. Given an image and a multiple-choice question about a specific taxonomy level (e.g., genus, family), first infer the most likely species from the image, then reason step-by-step through the taxonomy hierarchy to identify the correct label. Respond with only the letter corresponding to the correct answer.\nExample: if the image depicts *Oenothera curtiflora*, then the correct genus is *Oenothera*, family is *Onagraceae*, order is *Myrtales*, class is *Magnoliopsida*, phylum is *Tracheophyta*, and kingdom is *Plantae*.\nFor instance, if the question is:\n*Given the plant in the image, what is its taxonomic classification at the phylum level?*\nA. Chlorophyta\nB. Bryophyta\nC. Rhodophyta\nD. Tracheophyta\nYou should select option **D**, labeled with *Tracheophyta*."
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_file",
        type=str,
        default="/projectnb/ivc-ml/yuwentan/LLaVA-NeXT/QWEN_EVAL/bio/plant_1_latter.json",
    ...
    parser.add_argument(
        "--test_set",
        type=str,
        default="/projectnb/ivc-ml/yuwentan/LLaVA-NeXT/Evaluate_CLS/data/Plant/new_Plantae_with_random_choice.jsonl",
